...
class UpdateDatabase():
    def main(self):
        ...
        query = ("SELECT LinearInterpoMeasurement.Date, LinearInterpoMeasurement.RepID, " +
                 "LinearInterpoMeasurement.N2OFlux, LinearInterpoMeasurement.VWC, LinearInterpoMeasurement.SoilT, " +
                 "LinearInterpoMeasurement.LinMeasID, Replication.RepID, Replication.TreatmentID " +
                 "FROM LinearInterpoMeasurement INNER JOIN Replication ON " +
                 "LinearInterpoMeasurement.RepID = Replication.RepID " +
                 "ORDER BY LinearInterpoMeasurement.LinMeasID ASC")
                ...
                for replication in gn_replication:
                    ...
                    if 'precip.csv' not in os.listdir('GRACEnet/' + experiment + '/' + treatment + '/' + replication):
                        ...
                        for timeseries in gn_timeseries:
                            ...
                            print(current_flux)
                            current_flux = UpdateDatabase.truncate(current_flux, 2)
                            current_flux_high = current_flux + 0.01
                            current_flux_high = round(current_flux_high, 2)
                            current_flux = str(current_flux)
                            current_flux_high = str(current_flux_high)
                            current_vwc = round(current_vwc, 2)
                            current_soiltemp = round(current_soiltemp, 2)
                            current_vwc = str(current_vwc)
                            current_soiltemp = str(current_soiltemp)
                            print(current_date)
                            print(current_flux)
                            print(current_flux_high)
                            print(current_vwc)
                            ...
                            if flag == 1:
                                ...
                                missingdf = missingdf.append({'Experiment': experiment, 'Treatment': treatment,
                                                                ...
                                                                'Date': current_date, 'N2OFlux': current_flux},
                                                                 ignore_index=True)
                                continue
                            currentdf = lineardf.loc[lineardf['LinMeasID'] == current_linmeasid]
                            # Convert currentdf to a list
                            current_list = currentdf.values.tolist()
                            print(current_list)
                            current_repid = current_list[0][6]
                            current_treatmentid = current_list[0][7]
                            print(currentdf)
                            print(current_linmeasid)
                            print(current_repid)
                            print(current_treatmentid)
                            # Update the Replication table with the replication name
                            cnx = mysql.connector.connect(user='root', password='Kh18riku!',
                                                          host='127.0.0.1',
                                                          database='global_n2o')
                            cursor = cnx.cursor()
                            query = ("UPDATE Replication SET RepName = '" + replication + "' WHERE RepID = " +
                                     str(current_repid))
                            cursor.execute(query)
                            cnx.commit()
                            # Update the Treatment table with the treatment name
                            query = ("UPDATE Treatment SET TreatmentName = '" + treatment + "' WHERE TreatmentID = " +
                                        str(current_treatmentid))
                            cursor.execute(query)
                            cnx.commit()
                            cnx.close()
                    # If not multiple timeseries, get the flux.csv
                        ...
                        current_flux = UpdateDatabase.truncate(current_flux, 2)
                        current_flux_high = current_flux + 0.01
                        current_flux_high = round(current_flux, 2)
                        current_flux = str(current_flux)
                        current_flux_high = str(current_flux_high)
                        print(current_date)
                        print(current_flux)
